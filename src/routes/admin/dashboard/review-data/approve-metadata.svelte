<script lang="ts">
	...
	let checked_row = include_keys.map((key) => ({ name: key, disabled: true }));
	...
	$: save_draft =
		checked_row.some((f) => !f.disabled) || files_checked_row.some((f) => !f.disabled);
	...
	{#if !obj.approved}
		<Button
			...
			on:click={async (e) => {
				...
				modified = false;
				checked_row = checked_row.map((f) => ({ ...f, disabled: true }));
...
<form
	...
	class="grid gap-4"
	action="?/update&id={obj.id}&api_key={api_key}"
	method="POST"
	enctype="multipart/form-data"
>
	<div class="grid-auto-fill">
		{#each checked_row as { name, disabled }}
	...
	{#if api_key === 'species-metadata'}
		...
		<div class="grid-auto-fill">
			{#each files_checked_row as { name, disabled, file }}
	...
	{#if !obj.approved}
		<div class="flex gap-4 ml-auto">
			...
			{#if api_key === 'species-metadata'}
				{#if obj.cat_file}
					<Button type="submit" disabled={!save_draft || uploading}>
